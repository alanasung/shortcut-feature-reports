...
from introspect.configs.loader import register_schema
...
def _load_stage_registry(package: str = "introspect") -> dict:
...
def _run_stages(cfg: DictConfig, run_dir: Path) -> dict:
    ...
    if unavailable:
        ...
        print(f"      in introspect.stages: {unavailable}")
...
def main(cfg: DictConfig) -> None:
    ...
    from introspect.configs.loader import config_to_dict, save_resolved_config
    from introspect.models.device import get_device
    from introspect.utils.logging import configure_logging
    from introspect.utils.reproducibility import set_seed
    from introspect.utils.run_manifest import create_run_dir, save_run_metadata
