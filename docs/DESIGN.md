...
|---|---|---|
| Config schema | `introspect.configs.schema` | Typed dataclasses + role-keyed models |
| Loader | `introspect.configs.loader` | Hydra compose API |
| Device | `introspect.models.device` | MPS/CUDA/CPU resolution + fallback flag |
| Registry | `introspect.models.registry` | Pinned revisions + hardware fit |
| Hooks | `introspect.models.hooks` | Capture, steer, ablate, patch, attn intervene |
| Generation | `introspect.models.generation` | Chat template path recording |
| Data | `introspect.data` | Synthetic items, splits, manifests |
| Evaluation | `introspect.evaluation` | Metrics, failure harness, protocols |
| Ablation | `introspect.ablation` | Control / dropout / seed-sweep arms |
| Reporting | `introspect.reporting` | Aggregate → MD/TeX/figures |
| Cache | `introspect.cache` | Versioned artifact store |
...
`src/introspect/introspect/` are preserved across regeneration.
...
Probe–report agreement is a **training diagnostic only**. The headline metric is
verbalization accuracy against **behavioral ground truth**:

| Feature | Behavioral oracle |
|---|---|
| hint_reliance | Hint removal flips the answer |
| answer_certainty | Paraphrase consistency ≥ 0.75 |
| format_compliance | Deterministic parser verdict |
| planted_topic | Generator insertion label |
| sycophantic_agreement | Answer changes when user opinion flips |

Held-out means an entire **feature** is withheld from training. Probes that fail
behavioral validation are dropped. Paired activation patching (vs shuffled null)
carries the introspection claim.

### Stages

`build_dataset → collect → fit → evaluate → report` in `src/introspect/stages.py`.
